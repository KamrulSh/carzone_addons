...
class BalanceSheetView(models.TransientModel):
    ...
    def view_report(self, option, tag, lang):
        ...
        parent_list = list(set(parent_list))
        final_report_lines = []
        final_report_lines_list = []
        for rec in report_lines_move:
            if not rec['name'] in final_report_lines_list:
                if rec['report_type'] != 'accounts':
                    if rec['r_id'] in parent_list:
                        final_report_lines.append(rec)
                else:
                    final_report_lines.append(rec)
                final_report_lines_list.append(rec['name'])
    ...
    def get_acc_type(self, acc_id):
        acc_type = self.env['account.account'].search([('id','=',acc_id)])
        return acc_type.user_type_id.name
    ...
    def _get_report_values(self, data):
        ...
        acc_data = []
        for acc_id in account_res:
            account_details = {}
            get_acc = self.env['account.account'].search([('id','=',acc_id.get('account_id'))])
            account_details[get_acc.user_type_id.name] = acc_id
            acc_data.append(account_details)
        # print(acc_data)
        debit_total = 0
