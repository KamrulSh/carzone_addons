...
#        'account.invoice.line',
    ...
    gross_margin_id = fields.Many2one("account.analytic.line", string="Gross Margin ID")

    def create(self, vals):
        res = super(JobCostLine, self).create(vals)
        for val in vals:
            if val.get('analytic_id'):
                val_list = {
                        'name': val.get('description') or _("Job Cost"),
                        'account_id': val.get('analytic_id'),
                        'date': val.get('date') or fields.datetime.today(),
                        'unit_amount': val.get('product_qty') or val.get('hours'),
                        'ref': val.get('reference') or '',
                        'project_id': val.get('project_id'),
                        'product_uom_id': val.get('uom_id'),
                    }
                if val.get('job_type') == 'labour':
                    val_list.update({'amount': -(val.get('hours') * val.get('cost_price'))})
                else:
                    val_list.update({'amount': -(val.get('product_qty') * val.get('cost_price'))})

                rec = self.env['account.analytic.line'].create([val_list])
                res.gross_margin_id = rec.id
        return res

    def write(self, vals):
        res = super(JobCostLine, self).write(vals)
        if self.gross_margin_id:
            analytic_account_line_id = self.env['account.analytic.line'].search([('id', '=', self.gross_margin_id.id)])

            if not analytic_account_line_id.unit_amount:
                analytic_account_line_id.unit_amount = self.product_qty or self.hours

            if vals.get('description'):
                analytic_account_line_id.name = vals.get('description')

            if vals.get('date'):
                analytic_account_line_id.date = vals.get('date')

            if vals.get('product_qty'):
                analytic_account_line_id.unit_amount = vals.get('product_qty')
                analytic_account_line_id.amount = -(vals.get('product_qty') * self.cost_price)

            if vals.get('hours'):
                analytic_account_line_id.unit_amount = vals.get('hours')
                analytic_account_line_id.amount = -(vals.get('hours') * self.cost_price)

            if vals.get('cost_price'):
                if self.job_type == 'labour':
                    analytic_account_line_id.amount = -(self.hours * vals.get('cost_price'))
                else:
                    analytic_account_line_id.amount = -(self.product_qty * vals.get('cost_price'))

            if vals.get('reference'):
                analytic_account_line_id.ref = vals.get('reference')
            if vals.get('uom_id'):
                analytic_account_line_id.product_uom_id = vals.get('uom_id')
